...
### 配件（C3=3003，3PP）歸類
1. 自身 C4 對應類別（4007=CPU、4009=iPhone、4010=iPad、4039=Watch、4069=AirPods、4012/4022→iPhone）
   若在「同單主機類別」中 → 歸該類
2. 否則跟著同單唯一主機類別（例：通用線材在 Watch 單 → Watch配件；iPad 殼在 AirPods 單 → AirPods配件）
3. 無主機的純配件單 → 跟著**同 VIP 當日**買的主機類別；VIP 當日沒買主機 → **不列入統計**

### 搭售/零搭售台數
- 每「單據×類別」若有歸類到該類的配件 → 該類**只有 1 台**記「搭售」，同單同類其餘台數記「零搭售」
