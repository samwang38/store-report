...
## 3. 門市層級 KPI（`calc_metrics`，Sheet 6/16 使用）
...
- 獎金計算（Sheet 12 公式）：F=SA營收/(SA+D欄)、G=3PP/原廠比、J=SA毛利=E÷2÷1.05、K=H+I+J
...
- **YOY（Sheet 16/17）**：1/1 累積至截止日（可自訂 `yoyEnd`），對比前一年同日。
...
- `POST /api/generate` `{"shopId":"004","weekEnd":"YYYY-MM-DD"}` → 輪詢 `/api/status` → `/api/download` 拿 17-sheet Excel
...
## 附錄：DSS 搭售統計邏輯（Sheet 10/11，2026-06 反推驗證）
...
完全重現（當週 231/231 列、6 員工×5 機種×3 欄位 30/30 聚合全中）。實作：`server.py::epb_bundle_stats`。

### 機種判定
| 機種 | 條件 |
|------|------|
| CPU | C3=3001 且 C4∈{4001,4002} |
| iPhone | C3=3001 且 C4=4004 |
| iPad | C3=3001 且 C4∈{4005,4006,4041} |
| Watch | C3=3001 且 C4=4038 |
| AirPods | **C3=3002** 且 C4=4014 且 C6∈C6_AIRPODS（引擎常數） |

### 配件（C3=3003，3PP）歸類（2026-08-23 改版，見 v5 補充）
1. 跟著**同單「單價最高」的主機**的類別 — 不看配件自身 C4。
   （例：iPhone 17 Pro 38,000＋Watch S11 13,238 同單，連 Apple Watch 錶帶都算 iPhone 的配件）
2. 3001 主機層優先於 AirPods 機層：同單同時有 3001 主機與 AirPods 機時，只在 3001 主機中比價
3. 無主機的純配件單 → 跟著**同 VIP 當日**最高價主機的類別；VIP 當日沒買主機 → **不列入統計**

### 排除：會員產業別
- `poslinev_bi.OCCUPATION_ID`（＝POS_VIP_MAS.OCCUPATION_ID）為 **88 企業公司戶**、**99 通訊業（大量購買）**
  的會員，整張單不列入 3PP 統計。程式常數 `server.py::BUNDLE_EXCLUDE_OCC`

### 搭售/零搭售台數
- 每「單據×類別」若有歸類到該類的配件 → 該類**單價最高的 1 台**記「搭售」，同單同類其餘台數記「零搭售」
  （POS 組合販賣行為；同單 2 台 CPU＋8 配件仍是 1 搭售＋1 零搭售）
- 純配件單的配件可讓**同 VIP 同日同類別**尚無搭售的 1 台零搭售升級為搭售（跨單配對）
- 交易類型：A 銷售/H 尾款 正計、E 銷退 負計（沖銷自然相抵）；訂金 G 不出現在明細

### 衍生欄位（報表顯示）
- 搭售率 = 配件數 ÷ 搭售台數（顯示為倍數，四捨五入 2 位）
- 零搭售比例 = 零搭售 ÷（搭售＋零搭售）
- AA「3PP搭售率」= Σ配件 ÷ Σ搭售台數（五類合計）

### 區間定義（週報模板 Sheet 10/11）
- Sheet 10（週）：本期＝本週日～週六；歷史 3 列＝前 3 個完整週，標籤 W{財務季週次}
- Sheet 11（月）：本期＝本月 1 日～週結束日；歷史 3 列＝前 3 個完整日曆月
- 比對工具：`GET /api/bundle/preview?shopId=004&start=…&end=…&source=epb|dss|both`
- 報表填寫目前以 DSS 為準（EPB 版為驗證用；規則僅以一週樣本驗證，跨週期穩定性待觀察）

### v3 規則補充（2026-06-10 五週 155 數據點驗證後定版）
- **交易類型**：只計 A 銷售 / E 銷退 / H 尾款；**G 訂金、J 退訂不計**（DSS 明細不含）
- **配件歸類優先序**：同單 3001 主機 → 同單 AirPods 機 → 同 VIP 當日 3001 主機 → 當日 AirPods 機 → 都沒有則不計。
  每一層內：配件自身 C4 類別若在主機類別集合中→用自身；否則唯一主機類別→跟主機；多類別→用自身（可能不計）。
  例：Watch+AirPods 同單，AirPods 耳機套（4069）歸 **Watch** 配件（AirPods 機不參與第一層歸類）。
- **跨單配對**：純配件單（無主機）可讓同 VIP 同日同類別「還沒有搭售」的 1 台零搭售升級搭售；
  無會員（VIP 空/0）不做跨單配對。配件歸戶跨員工（A 員工賣的配件可配 B 員工賣的主機，配件數記 A）。
- **驗證結果**：5/3–6/6 五週、155 個（員工×機種×3欄）數據點，**154 吻合（99.4%）**。
  唯一差異：2026-05-06 一張 mini＋點數折抵單（7572-004e02-20260506172924-0009）整張不在 DSS 明細
  （同日其他門市同型號都在），判定為 **DSS 端漏單**，EPB 較準。
- **正式報表已切換 EPB 為主、DSS 備援**（`load_bundle_stats`，單次大區間查詢切片提速）。
  local_config.json 設 `"bundleSource": "dss"` 可強制改回 DSS 為主。

### v4 補充：認證機＋驗證總結
- **認證機/整新機也算主機**：品牌∈{881,885,886,888} 時 bypass C3（實例：C3=3035 整新 iPhone、C3=3033 認證 MacBook），
  類別仍依 C4 判定。已納入 `_bundle_mach_cat`。
- **最終驗證**（士林，2026-03-01～06-06）：
  - 週層級（5 週 × 員工 × 機種）：155 點對 154
  - 月層級（3/4/5 月店合計）：45 點對 40
  - **所有差異（6 點）皆源自 5 張「整張不在 DSS 明細」的單據**（多為 mini＋2點折1元；
    但同結構單據如 2026-05-31 的 mini＋點折 DSS 又有收），已逐張用 DSS 明細查無、判定為 DSS 端 ETL 漏單，
    EPB 數字較完整。確認過的漏單：20260302172735-0009、20260320160344-0009、20260501123203-0001、
    20260506172924-0009（另 3 月 iPhone +1/+1/+1 推定同類）。

### v5 補充：最高價主機歸屬＋會員產業排除（2026-08-23 定版）
兩項規則變更，皆以 DSS 主站逐日比對確認：

1. **配件掛「同單最高價主機」**（取代舊的「配件自身 C4 優先」）。實證：
   - 2026-07-15 單 7573-004e03-20260715213821-0017：iPhone 17 Pro 37,998＋iPad Air 27,048，
     3 個 iPad 殼＋3 個 iPhone 殼 → DSS 6 個配件全掛 iPhone，**iPad 記零搭售**
   - 2026-07-14 單 …20260714150042-0007：iPhone 38,000＋Watch 13,238，含 Apple Watch 錶帶（C4=4039）
     → DSS 錶帶也算 iPhone 配件，Watch 零搭售
   - 2026-07-07：iPhone＋Watch 同單，另有同會員同日純配件單（2 條錶帶）→ DSS 一樣掛 iPhone，
     故跨單配對層也用「最高價主機」
2. **排除會員產業 88/99**（`OCCUPATION_ID`，對照表 `VIP_OCCUPATION`：88=企業公司戶、99=通訊業大量購買）。
   **修正 v4 的結論**：v4 判定為「DSS ETL 漏單」的 20260501123203-0001、20260506172924-0009 等，
   實際上都是 occupation 99/88 的客戶單，DSS 是「刻意排除」而非漏單。

**驗證**（逐日 × 員工 × 機種 × m/s/ms 三欄，對 DSS 3PP_aggregate_E）：

| 門市 | 期間 | 舊規則 | 新規則 |
|---|---|---|---|
| 士林 004 | 2026-05-01～08-21（113 天） | 1507/1530 = 98.50% | **1515/1517 = 99.87%** |
| 裕隆城 068 | 2026-07-01～08-21（52 天） | 973/999 = 97.40% | **984/992 = 99.19%** |
| 阿波羅 046 | 2026-07-01～08-21（52 天） | 951/980 = 97.04% | **961/965 = 99.59%** |

以報表實際使用的區間（週／月）比對士林 6 週＋3 個月共 553 點：**551 點吻合（99.64%）**，
6 週與 5、6 月全數 100%。唯一殘差是 2026-07-04 的「買 Watch→退貨→再買」單（m/ms 各差 1，台數總和相同）：
DSS 把退貨那台算零搭售 −1、原單算搭售，EPB 兩台都歸搭售側。試過改跨單升級規則配合，整體吻合率反而下降，不採用。

**DSS 資料延遲一天**：2026-08-23 查 8/22 的 DSS 回傳全空（EPB 有 11 個數據點）。
週報若在週日製作、區間到週六，用 DSS 當來源會漏掉最後一天 — 這是「強制使用 DSS」不建議勾選的主因。
